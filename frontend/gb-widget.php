	...
	class GwolleGB_Widget extends WP_Widget {
		...
		function widget($args, $instance) {
			...
			if ( (int) $postid > 0 ) {
				...
				$raquo = ' <a href="' . $permalink . '" title="' . __('Click here to get to the guestbook.', 'gwolle-gb') . '">&raquo;</a>';
			}

			$widget_html .= '<ul class="gwolle_gb_widget ' . $slider_class . '">';
			$counter = 0;
			...
			if ( is_array( $best ) && !empty( $best ) ) {
				foreach ($best as $entry_id) {
					...
					if ( isset($entry_id) && $entry_id > 0 ) {
						...
						$widget_html .= '<span class="gb-entry-content">' . $entry_content . $raquo;
